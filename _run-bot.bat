...
rem    代理不认识 203.0.113.10 就会失败，表现为「模型全部超时、机器人哑掉」。
rem    实测：不加 NO_PROXY，回归里 5 个测试套件全挂。
rem ⚠️⚠️ 2026-09-16 深夜：**这三行注释掉了** —— 当时「机器人忽然不能聊天」就是这个原因：
rem    机器人被强制走本地代理 203.0.113.10，而**代理软件没开** →
...
rem set "HTTP_PROXY=http://203.0.113.10"
rem set "HTTPS_PROXY=http://203.0.113.10"
rem set "NODE_USE_ENV_PROXY=1"
rem ⚠️⚠️ 2026-09-16 深夜：**改成"自动选出口"**（用户要求：「要能自动切换网络，比如关掉代理」）。
rem    那晚的故障：这里写死 `HTTPS_PROXY=203.0.113.10`，而**代理软件没开**
...
node -e "const n=require('net');const s=n.connect(7890,'203.0.113.10');s.setTimeout(400);s.on('connect',()=>{console.log(1);s.destroy()});s.on('timeout',()=>{console.log(0);s.destroy()});s.on('error',()=>{console.log(0)})" > "%PCHK%" 2>nul
set "PROXYUP=0"
if exist "%PCHK%" set /p PROXYUP=<"%PCHK%"
del "%PCHK%" >nul 2>&1
if "%PROXYUP%"=="1" (
  set "HTTP_PROXY=http://203.0.113.10"
  set "HTTPS_PROXY=http://203.0.113.10"
