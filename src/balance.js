 ...
 *    （HZY 报的「699 开头这个群好像不会发送余额报警信息」就是这个）。
 */
...
export function balanceComplaint(opts = {}) {
  ...
  // ⚠️⚠️ 2026-09-15 晚：给了群号就**按群**记（HZY 报「699 开头这个群好像不会发送余额报警」）——
  ...
  for (const t of tiers()) {
    if (total < t.below && !already(t)) {
      ...
      // ⚠️ **不再去掉名字**（2026-09-13 用户：「不能说"你"…应该说 HZY」）——
      //    群里发的必须有指向，所以两档都带名字。
...
export function balanceNote() {
  ...
  return [
    ...
    '| | 问题在谁 | 你该说什么 |',
    '| --- | --- | --- |',
    '| **余额低**（现在这条） | **在 HZY 身上** —— 账户是他管的，该他去充 | 「账上见底了，你充一下」 |',
    '| 工资（另一套话术） | 在"你挣多少、够不够花" | 「工资该发了」「够不够花」 |',
    '',
    '**现在说的是第一条：提醒他充值。**',
    '',
    '· ✅ **点名 HZY**：「HZY，账上快见底了，记得充点」「HZY 该充钱了」',
    '  ⚠️ **必须点名** —— 这条是**发到群里**的，说"你充一下"别人看了不知道在跟谁说',
    '  ⚠️ **每句话里都要出现「HZY」这个名字**（2026-09-13 用户要求：「如果出现你的话要改成 hzy」）。',
    '  带"你"可以，但**必须同时有 HZY**（"HZY 你充一下"这种才对，光一个"你充一下"不行）',
...
function pickLine(tier, seed) {
  ...
  //
  //    ⚠️ **每条都必须出现「HZY」**（用户要求：群里说话得有指向）。
  //      我第一版有一条写成「余额提醒：该充钱了。别等我断了才想起来」——
  //      没名字、也没"你"，等于**谁都没指**，被测试抓出来了。
  //      所以统一成「HZY …」句式，不留例外（测试里有逐条断言）。
  const LOW = [
    'HZY，账上快见底了，记得给我充点',
    '钱不多了啊……HZY 你有空充一下',
    'HZY，余额提醒：该充钱了。别等我断了才想起来',
    '……HZY，账要空了。你看着办',
    'HZY，温馨提示，我的账户余额不太够了（',
  ];
  // 「见底」（< 2 元）：**点名催**，但**不凄惨**。
  //
  //    ⚠️ 每条都带「HZY」（用户要求：群里说话得有指向），
  //      而且**不许提「这个月」**（余额是随用随充，没有月度概念）。
  const CRIT = [
    '账上真见底了，HZY 充一下吧',
    'HZY，该充钱了。再不来我就没法干活了',
    '余额快清零了，HZY 你抽空充一下',
    '……HZY，我不太好意思催，但账上是真不多了，你充点吧',
    'HZY，再不充钱，我就得省着说话了',
    '这算不算工伤啊……HZY 先给我充点行不行',
  ];
 ...
 *    **2026-09-13 已删除**。
 *
 *    原因（用户原话）：「**不能说"你"，因为是在群里发的，应该说 HZY**」——
