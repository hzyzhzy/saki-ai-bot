...
export class Bot {
  ...
  shouldJoinChat(event, { stickerIsNew = false, echoSticker = null } = {}) {
    ...
    //    真实案例：HZY 在群里回某人「@mmmawa 我看看有什么」，机器人却接了这条，
  ...
  async handle(event, meta = {}) {
    ...
    if (ackAfter > 0 && !skipAck) {
      ackTimer = setTimeout(() => {
        ...
        quickAck({
          name: event.sender?.card || event.sender?.nickname || '',
          text: promptText,
          extra: msg.hasMediaPlaceholder(promptText) ? '对方发了图/表情，你在看图' : '',
        })
          .then((t) => {
            ackText = t || '';
    ...
    try {
      ...
      if (!stopped && buffer.trim()) {
        const fake = detectFakeRecall(stripMarkers(buffer).trim());
        if (fake) {
          log.warn(`[编造群史] 拦下一条：${JSON.stringify(stripMarkers(buffer).trim().slice(0, 80))}`);
