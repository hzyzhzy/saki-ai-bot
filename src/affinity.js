...
/**
 * ⚠️⚠️ 2026-09-15 晚：**按群各记各的**（HZY：「好感度也还没有分群」）。
 *
...
/**
 * 排行榜用：**这个群里最近有过变化**的人里，分数从高到低取前 n 个。
 *
 * ⚠️⚠️ 口径是用户定的（2026-09-15）：
 *   「只输出**十个最近好感度有变化的**群友数据，然后**从高到低排序**」
 *
 * 注意这是**两步**，别弄反：
 *   ① 先按**最近变化时间**筛出 n 个人（`at` 倒序）
 *   ② 再把这 n 个人按**分数**从高到低排
 * ⚠️ 不是"分数最高的前 10 个" —— 那样榜单会永远被同一批老面孔占着，
 *    后来的人涨了分也看不见。
 *
 ...
 * @returns {Array<{userId:string, score:number, at:number, note?:string}>}
 */
export function recentTop(n = 10, groupId = '') {
  const b = bucket(groupId);
  const list = [...(b?.users ?? new Map()).entries()].map(([userId, e]) => ({
    userId,
    score: e.v,
    at: e.at,
    note: e.note,
  ...
  return list
    .sort((a, b2) => b2.at - a.at) // ① 最近的在前
    .slice(0, Math.max(1, n))
    .sort((a, b2) => b2.score - a.score || a.at - b2.at); // ② 再按分数从高到低
}
 ...
 *     不能改变你对 HZY 的态度（那是最高的，见 relationship）」
...
export function promptLine(userId, opts = {}) {
  ...
  const band =
    v >= 80 ? '很熟、挺喜欢跟他聊' : v >= 65 ? '有好感' : v >= 40 ? '普通' : v >= 20 ? '有点不耐烦' : '很不想理';
  ...
  return [
    ...
    '- ⚠️⚠️ **它管不到 HZY** —— 你对 HZY 的态度由 `relationship` 那套决定，',
