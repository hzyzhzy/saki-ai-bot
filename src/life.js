...
import { personaText, castBlock } from './knowledge.js';

const FILE = join(KNOWLEDGE_DIR, 'life-events.md');
...
/**
 * **预览一条**（HZY 2026-09-15：「日常事件也加一个预览和立即发送」）。
 *
...
const LIFE_SYSTEM = `你是「丰川祥子」（自称 Saki）。现在要你**主动在群里说一句**，内容是你今天遇到的一件小事。
...
3. ⚠️⚠️ **群里的人不知道这件事**（HZY 2026-09-15 截图反馈：「这个很明显，群友看不懂在说什么」）。
   这是这条消息**最容易犯的错**，单独列一条：
...
10. ⚠️⚠️ **听你说话的人是群友，不是那些出场的人。**
   初华/睦/海铃/若麦她们是**你在跟群友讲的事里的人** ——
   🚫 别写成你在对她们说话、吩咐她们、或者跟她们你一句我一句。
   （真实踩过：HZY 截图反馈「最后一句有点问题，说话对象应该是群友，
     而不是对祥子队友说的」——那次是把给队友下的命令直接发到群里了。）
 ...
 *
 * ⚠️ HZY 2026-09-15：「**挡位 2 不能进事件系统，只有 1 才能设置**」。
